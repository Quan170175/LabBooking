...
export default function ManagerApprovalCard({
  ...
  const slotSummary = useMemo(() => {
    ...
    const lines = Object.keys(groups).map(date => 
      `${formatDate(date)} (${groups[date]} ca)`
    );
      ...
      <View style={styles.body}>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.rowText}>
            <Text style={{fontWeight: '700'}}>{booking.labRoomResponse?.labName}</Text>
            <Text style={{color: '#64748B'}}> - {booking.labRoomResponse?.location}</Text>
          </Text>
        </View>
        ...
        {booking.courseResponse && (
          <View style={styles.row}>
            ...
            <Text style={styles.rowText} numberOfLines={1}>
              <Text style={{fontWeight: '700'}}>{booking.courseResponse.courseCode}</Text> 
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.rowText}>
            Sĩ số: {booking.numberOfParticipants} / {booking.labRoomResponse?.maximumLimit} sinh viên
          </Text>
        </View>
        ...
        <View style={[styles.row, { alignItems: 'flex-start' }]}>
          ...
          <Text style={[styles.rowText, { color: '#0F172A', fontStyle: 'normal' }]}>
            {slotSummary}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  badgeText: { fontSize: 11, fontWeight: "600", color: "#475569", textTransform: 'uppercase' },
