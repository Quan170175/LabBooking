...
const api_getUnavailableSlots = async (roomId: string, startDate: string, endDate: string) => {
  try {
...
export default function BookSlots() {
  ...
  const [baseSlots, setBaseSlots] = useState<{ dayIndex: number; slotId: string }[]>([]);
  const [numWeeks, setNumWeeks] = useState(1);
  const [selectedSlots, setSelectedSlots] = useState<{ date: string; slotId: string; isConflict: boolean }[]>([]);
  ...
  useEffect(() => {
    ...
    const loadBooked = async () => {
      ...
      try {
        ...
        const data = await api_getUnavailableSlots(roomId as string, start, end);
        setUnavailableSlots(data); 
      } finally { setIsLoadingUnavailable(false); }
    };
    loadBooked();
  }, [roomId, weekDates]); 
  ...
  useEffect(() => {
    ...
    const newSelectedSlots: { date: string; slotId: string; isConflict: boolean }[] = [];
    ...
    for (let week = 0; week < numWeeks; week++) {
      for (const baseSlot of baseSlots) {
        ...
        const blockedSlot = unavailableSlots.find(s => isSameDate(s.date, dateString) && s.slotId === baseSlot.slotId);
        const existingPriority = blockedSlot?.priority ?? 2;
        const isMaintenance = existingPriority === 0;
        const canOverride = blockedSlot && !isMaintenance && myPriority === 1 && existingPriority === 2;
  ...
  }, [baseSlots, numWeeks, anchorDate, unavailableSlots, isRecurring, allSlots, weekDates, myPriority, currentMonday]);

  // --- HÀM QUÉT TRÙNG TƯƠNG LAI (QUAN TRỌNG) ---
  const checkFutureConflicts = async (dayIndex: number, slotId: string, startMon: Date, weeksToCheck: number) => {
    ...
    console.log(`Scanning future: ${startStr} -> ${endStr} for ${weeksToCheck} weeks`);
    const busyData = await api_getUnavailableSlots(roomId as string, startStr, endStr);
    ...
    for (let w = 0; w < weeksToCheck; w++) {
        ...
        if (conflictItem) {
            console.log(`Conflict found at ${targetDateStr}, Priority: ${conflictItem.priority}`);
            const existingPriority = conflictItem.priority ?? 2;
            const isMaintenance = existingPriority === 0;
            
            // Chỉ cho phép đè nếu: Không phải bảo trì + Tôi là 1 + Họ là 2
            const canOverride = !isMaintenance && myPriority === 1 && existingPriority === 2;
  ...
  const handleChangeNumWeeks = async (newWeekValue: number) => {
    ...
    if (clampedWeeks > numWeeks) {
        ...
        for (const slot of baseSlots) {
            ...
            const checkResult: any = await checkFutureConflicts(slot.dayIndex, slot.slotId, startPoint, clampedWeeks);
  ...
  const toggleRecurringSlot = async (dayIndex: number, slotId: string) => {
    ...
    const unavailableSlotThisWeek = unavailableSlots.find(s => isSameDate(s.date, dateString) && s.slotId === slotId);
    if (!checkCanSelect(unavailableSlotThisWeek)) return;

    const isSelecting = !baseSlots.some(s => s.dayIndex === dayIndex && s.slotId === slotId);

    if (isSelecting) {
        setIsCheckingConflict(true);
        const checkResult: any = await checkFutureConflicts(dayIndex, slotId, startPoint, numWeeks);
        setIsCheckingConflict(false);
        ...
        if (checkResult.isConflict) {
            ...
            Alert.alert("Trùng lịch", `Slot ${slotInfo?.label} vào ${dayName} ngày ${dateDisplay} ${checkResult.reason}.`);
            return;
        }
    }
    ...
    const idx = newBaseSlots.findIndex(s => s.dayIndex === dayIndex && s.slotId === slotId);
  ...
  const toggleMultiSlot = (date: Date, slotId: string) => {
    ...
    const unavailableSlot = unavailableSlots.find(s => isSameDate(s.date, dateString) && s.slotId === slotId);
    if (!checkCanSelect(unavailableSlot)) return;

    let isBooked = !!unavailableSlot; 
    const exists = selectedSlots.find(s => s.date === dateString && s.slotId === slotId);
    
    if (exists) {
      setSelectedSlots(selectedSlots.filter(s => !(s.date === dateString && s.slotId === slotId)));
    } else {
      if (selectedSlots.length >= maxSlotsTotal) {
        Alert.alert("Đã đạt giới hạn", `Tối đa ${maxSlotsTotal} slot.`);
        return;
      }
      setSelectedSlots([...selectedSlots, { date: dateString, slotId, isConflict: isBooked }]);
    }
  };
  ...
  const goToDevices = async () => {
    ...
    if (selectedSlots.length === 0) {
      Alert.alert("Chưa chọn slot", "Vui lòng chọn ít nhất một giờ học/làm việc.");
      return;
    }
    ...
    try {
      ...
      // 5. Lưu xuống Storage
      await AsyncStorage.setItem("currentBooking", JSON.stringify(updatedBookingData));
      ...
      Alert.alert("Lỗi hệ thống", "Không thể lưu dữ liệu đặt phòng. Vui lòng thử lại.");
    }
  };
  ...
  const selectionInfo = useMemo(() => {
    if (isRecurring) {
      ...
      return <><Text style={{ fontWeight: "bold" }}>{baseSlots.length}</Text> slot/tuần. Tổng: <Text style={{ fontWeight: "bold" }}>{selectedSlots.length}</Text> / {maxSlotsTotal} slots</>;
    }
    return <>Đã chọn: <Text style={{ fontWeight: "bold" }}>{selectedSlots.length}</Text> / {maxSlotsTotal} slot</>;
  ...
  if (!roomDetails) return <View style={styles.centered}><Text>Lỗi tải dữ liệu</Text></View>;

  const startDateStr = weekDates[0] ? `${weekDates[0].getDate()}/${weekDates[0].getMonth() + 1}` : "";
  const endDateStr = weekDates[6] ? `${weekDates[6].getDate()}/${weekDates[6].getMonth() + 1}/${weekDates[6].getFullYear()}` : "";
  const dateRange = `${startDateStr} - ${endDateStr}`;
  const weeksDiffFromStart = Math.floor((currentMonday.getTime() - (anchorDate || currentMonday).getTime()) / (7 * 24 * 60 * 60 * 1000));
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <BookingPageHeader icon={headerIcon} title={roomDetails.labName} subtitle={`${roomDetails.location} - ${roomDetails.maximumLimit} chỗ`} />
      ...
      <View style={styles.calendarContainer}>
        <View style={styles.weekdaysHeader}>
          {weekDates.map((date, idx) => (
            <View key={idx} style={styles.dayHeader}><Text style={styles.dayNameText}>{weekdays_short[idx]}</Text><Text style={styles.dateNumText}>{date.getDate()}</Text></View>
          ))}
        </View>

        {(isLoadingUnavailable || isCheckingConflict) && <View style={styles.slotsLoading}><ActivityIndicator size="small" color="#EA580C" />{isCheckingConflict && <Text style={styles.checkingText}>Kiểm tra lịch...</Text>}</View>}

        <View style={[styles.slotsGrid, (isLoadingUnavailable || isCheckingConflict) && styles.hidden]}>
          {weekDates.map((date, dayIndex) => (
            <View key={dayIndex} style={styles.dayColumn}>
              {allSlots.map((slot) => {
                const dateString = formatDateLocal(date);
                const unavailableSlot = unavailableSlots.find(s => isSameDate(s.date, dateString) && s.slotId === slot.id);
                const existingPriority = unavailableSlot?.priority ?? 2;
                const isMaintenance = existingPriority === 0;
                const canOverride = unavailableSlot && !isMaintenance && myPriority === 1 && existingPriority === 2;
                const isDisabled = unavailableSlot && !canOverride;
                const isRestricted = isTimeRestricted(date, slot.startTime);

                let isSelected = false;
                if (isRecurring) {
                    const isInSelectedWeekRange = weeksDiffFromStart >= 0 && weeksDiffFromStart < numWeeks;
                    if (isInSelectedWeekRange) isSelected = baseSlots.some(s => s.dayIndex === dayIndex && s.slotId === slot.id);
                } else {
                    isSelected = selectedSlots.some(s => s.date === dateString && s.slotId === slot.id);
                }
                ...
                if (isSelected) {
                  slotStyle = styles.selectedSlot; textStyle.push(styles.selectedSlotText);
                } else if (canOverride) {
                  slotStyle = styles.overrideSlot; textStyle.push(styles.overrideSlotText);
                } else if (isRestricted) {
                    slotStyle = styles.pastSlot; textStyle.push(styles.unavailableSlotText);
                } else if (isDisabled) {
                  if (isMaintenance) {
                    slotStyle = styles.maintenanceSlot; textStyle.push(styles.maintenanceSlotText);
                  } else {
                    slotStyle = styles.unavailableSlot; textStyle.push(styles.unavailableSlotText);
                  }
                }

                return (
                  <TouchableOpacity key={slot.id} onPress={() => handleToggleSlot(date, slot.id, dayIndex)} disabled={(!isRecurring && isDisabled) || isRestricted} style={[styles.slotButton, slotStyle]}>
      ...
      <View style={styles.legend}>
        <View style={styles.legendItem}><View style={[styles.legendBox, styles.availableSlot]} /><Text style={styles.legendText}>Trống</Text></View>
        ...
        <View style={styles.legendItem}><View style={[styles.legendBox, styles.unavailableSlot]} /><Text style={styles.legendText}>Đã đặt</Text></View>
        ...
        {isPriority && <View style={styles.legendItem}><View style={[styles.legendBox, styles.overrideSlot]} /><Text style={styles.legendText}>Ghi đè</Text></View>}
      </View>
      ...
      {isRecurring && (
        <View style={styles.weekSelectorContainer}>
           ...
           <View style={styles.weekSelectorControls}>
             <TouchableOpacity onPress={() => handleChangeNumWeeks(numWeeks - 1)} disabled={numWeeks <= 1} style={[styles.weekNavButton, numWeeks <= 1 && styles.disabledButton]}><Text style={styles.weekNavText}>-</Text></TouchableOpacity>
             <Text style={styles.weekCountText}><Text style={{ fontWeight: "bold" }}>{numWeeks}</Text>{maxWeeksAllowed > 0 ? ` / ${maxWeeksAllowed}` : ""} tuần</Text>
             <TouchableOpacity onPress={() => handleChangeNumWeeks(numWeeks + 1)} disabled={numWeeks >= (maxWeeksAllowed || 20)} style={[styles.weekNavButton, numWeeks >= (maxWeeksAllowed || 20) && styles.disabledButton]}><Text style={styles.weekNavText}>+</Text></TouchableOpacity>
           </View>
        </View>
      )}

      <View style={styles.footer}>
        <Text style={styles.selectionText}>{isRecurring ? `${baseSlots.length} slot/tuần. Tổng: ${selectedSlots.length} slots` : `Đã chọn: ${selectedSlots.length} / ${maxSlotsTotal} slot`}</Text>
        <BookingButton label="Tiếp theo" onPress={goToDevices} disabled={selectedSlots.length === 0} />
      </View>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  calendarNav: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 8, paddingVertical: 12, marginBottom: 16, backgroundColor: "white", borderRadius: 16, borderWidth: 1, borderColor: "#FFE8DA" },
  navButton: { padding: 8 },
  dateRangeText: { fontSize: 14, fontWeight: "600", color: "#C2410C" },
  calendarContainer: { backgroundColor: "white", borderRadius: 16, padding: 12, borderWidth: 1, borderColor: "#FFE8DA", position: "relative" },
  weekdaysHeader: { flexDirection: "row", justifyContent: "space-between", marginBottom: 12, paddingHorizontal: 4 },
  ...
  slotsLoading: { position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(255, 255, 255, 0.85)", justifyContent: "center", alignItems: "center", zIndex: 10, borderRadius: 16 },
  checkingText: { marginTop: 8, color: "#EA580C", fontWeight: "500", fontSize: 12 },
  hidden: { opacity: 0.3 },
  ...
  overrideSlot: { backgroundColor: "#FFF1F2", borderColor: "#FDA4AF", borderStyle: 'dashed' }, 
  overrideSlotText: { color: "#BE123C", fontWeight: "600" },
  pastSlot: { backgroundColor: "#E2E8F0", borderColor: "#CBD5E1", opacity: 0.7 },
  conflictSlot: { backgroundColor: "#FFFBEB", borderColor: "#FBBF24" },
  conflictSlotText: { color: "#B45309", fontWeight: "600" },
  legend: { marginTop: 16, display: "flex", flexDirection: "row", flexWrap: "wrap", justifyContent: "space-around", columnGap: 8, rowGap: 12 },
  legendItem: { display: "flex", flexDirection: "row", alignItems: "center", gap: 8 },
  legendBox: { width: 16, height: 16, borderRadius: 4, borderWidth: 1 },
  legendText: { fontSize: 13, color: "#475569" },
  weekSelectorContainer: { marginTop: 20, padding: 12, backgroundColor: "white", borderRadius: 16, borderWidth: 1, borderColor: "#FFE8DA", flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  weekSelectorLabel: { fontSize: 15, fontWeight: "500", color: "#0F172A" },
  weekSelectorControls: { flexDirection: "row", alignItems: "center", gap: 12 },
  weekNavButton: { width: 32, height: 32, backgroundColor: "#FFEDD5", borderRadius: 8, justifyContent: "center", alignItems: "center" },
