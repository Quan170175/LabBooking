...
export default function ChangeSlotsScreen() {
  ...
  const handleToggleSlot = (
    ...
    isBusyByOthers: boolean
  ) => {
    if (isRestricted) {
      Alert.alert(
        "Không hợp lệ",
        "Slot này đã diễn ra hoặc quá gần giờ hiện tại."
      );
      return;
    }
    if (isMaintenance) {
          ...
          {weekDates.map((date, i) => (
            <View key={i} style={styles.dayColumn}>
              {allSlotsTemplate.map((slot) => {
                ...
                const isMyOriginal = bookingDetail?.slots?.some(
                  (s: any) =>
                ...
                const isBusyByOthers =
                  isInSystemBusy && !isMyOriginal && !isMaintenance;
